        ...
        <div class="container mx-auto px-4">
            <div class="max-w-xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
                <div class="bg-indigo-600 p-4">
                    <h1 class="text-white text-xl font-bold">{{ __('Chỉnh sửa đánh giá') }}</h1>
                </div>
                    ...
                    <form method="POST" action="{{ route('review.update', $review->id) }}">
                                    ...
                                    <div>
                                        ...
                                        <label for="rating{{ $i }}" class="cursor-pointer block p-2">
                                            <svg xmlns="http://www.w3.org/2000/svg"
                                                class="h-8 w-8 {{ old('rating', $review->rating) >= $i ? 'text-yellow-400' : 'text-gray-300' }}"
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="comment"
                                ...
                                name="comment" rows="5" placeholder="Hãy chia sẻ trải nghiệm của bạn về phòng trọ này...">{{ old('comment', $review->comment) }}</textarea>
                            @error('comment')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
